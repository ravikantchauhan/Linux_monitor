...
echo "Server name or Host name :<strong>$HOST</strong><br>
	Server loacl ip :<strong>$(hostname -I | awk '{ print $1 }')</strong><br>
	Server serial number :<strong>$(dmidecode -t system | grep Serial |awk '{print $3'})</strong><br>
	CPU <strong>$(lscpu | egrep 'Model name')</strong><br>
	Total number of core : <strong>$(nproc)</strong><br>
	SSD or HDD  : <strong>$(cat /sys/block/sda/queue/rotational)</strong><br>
...
echo $line | awk '{print $3}' >> $WEB_DIR/report.html
echo "</td></tr>" >> $WEB_DIR/report.html
done < <(ps -eo %mem,%cpu,comm --sort=-%mem | head -n 10)
echo "</table></BODY></HTML>" >> $WEB_DIR/report.html
##################################################
